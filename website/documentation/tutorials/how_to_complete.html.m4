...
-Dcom.sun.management.jmxremote.authenticate=false
    ...
    <div class="commandDiv">
      ...
      <h4><a name="HighlightingHotPath" id="HighlightingHotPath"></a>Code Hightlighting &amp; Hot Path</h4>
      <p>The source code is hightlighted according to the measured data. This is only available if the source file hasn't changed since the measurement was done. As you can see in the screeshot below, the <code>onQuit(...)</code> and <code>onSaveAs(...)</code> methods were not executed during the measurement of the SUT.</p>
      <p>The hot path of the source file is shown on the left side of the editor. The more often a statement was executed, the more red the color becomes. The <code>onNewBook(...)</code> method was exectuted more often than other methods, so the color is red.</p>
