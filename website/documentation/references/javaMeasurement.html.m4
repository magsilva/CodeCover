...
<!-- TODO <div class="content" id="DIV1" onclick="return DIV1_onclick()"> -->
    ...
    <p> All coverage data collected during a test run is called a test session. You can
      subdivide this test session into test cases. There is a number of methods to do
      this.</p>
    ...
    <p> You can use specifc comments in source files, that will be instrumented. These comments
      will be translated into method calls during the instrumentation process. For this
      reason, it is important, that you place these comments at such positions in your
      source files, where a method call is allowed. Otherwise you will receive compiler
      errors.</p>
...
// finishTestSession();</code></pre>
    ...
    <p>Getting to know, whether <code>a</code> is definitely assigned before its usage is very complex after instrumentation. For this reason, CodeCover <b>does not</b> consider boolean expression, where occurs at least one assignment (<code>a = b</code>) operator.</p>
    <p>The coverage measurement under Java is <b>not</b> threadsafe. For this reason, the number of executions for a statement might not be correct, when your programm uses multithreading. Nevertheless, CodeCover gets to know when a statement is covered correctly.</p>
