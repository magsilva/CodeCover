...
/**
 * This {@link ManualRedundancyView} is a view in eclipse. It provides the user with
 * the opportunity to identify a test case as a redundant one. This manual redundancy
 * detection effects on redundancy metrics and redundancy graph.
 * 
 ...
 * @version 1.0 
 */
public class ManualRedundancyView extends ViewPart
...
//                | SWT.READ_ONLY);
        ...
        // create viewer
        this.viewer = RedundancyViewerFactory.newContainerCheckedTreeViewer(
                parent,
                false,
                new Runnable() {
                    public void run() {
                        CodeCoverPlugin.getDefault().getTSContainerManager()
                                .removeListener(ManualRedundancyView.this);
                    }
                });
    	...
    	if(Criterion.compareTo("Condition") != 0){
    		for(int i = 0; i < wholeTestCases.size(); i++)
    		{
    			...
    			if(Criterion == "Statement"){
    				if(!SuiteR.isNaN()){
    					...
    					wholeTestCases.get(i).setSatementRedundancy(bd.doubleValue());
    				}
    				else
    					wholeTestCases.get(i).setSatementRedundancy(SuiteR);
    				wholeTestCases.get(i).setSatementCoveredItem(ts);
    			}
    			else if(Criterion == "Branch"){
    				if(!SuiteR.isNaN()){
    					BigDecimal bd = new BigDecimal(SuiteR);
    					bd = bd.setScale(2,BigDecimal.ROUND_HALF_UP);
    					wholeTestCases.get(i).setBranchRedundancy(bd.doubleValue());
    				}
    				else
    					wholeTestCases.get(i).setBranchRedundancy(SuiteR);
    				wholeTestCases.get(i).setBranchCoveredItem(ts);
    			}
    			else if(Criterion == "Loop"){
    				if(!SuiteR.isNaN()){
    					BigDecimal bd = new BigDecimal(SuiteR);
    					bd = bd.setScale(2,BigDecimal.ROUND_HALF_UP);
    					wholeTestCases.get(i).setLoopRedundancy(bd.doubleValue());
    				}
    				else
    					wholeTestCases.get(i).setLoopRedundancy(SuiteR);
    				wholeTestCases.get(i).setLoopCoveredItem(ts);
    			}
    			else if(Criterion == "All"){
    				if(!SuiteR.isNaN()){
    					BigDecimal bd = new BigDecimal(SuiteR);
    					bd = bd.setScale(2,BigDecimal.ROUND_HALF_UP);
    					wholeTestCases.get(i).setTotalRedundancy(bd.doubleValue());
    				}
    				else
    					wholeTestCases.get(i).setTotalRedundancy(SuiteR);
    				wholeTestCases.get(i).setTotalCoveredItem(ts);
    			}
    		}
    	}
    	else{
    		for(int i = 0; i < wholeTestCases.size(); i++)
            {
                ...
                if(!SuiteR.isNaN()){
                	BigDecimal bd = new BigDecimal(SuiteR);
                	bd = bd.setScale(2,BigDecimal.ROUND_HALF_UP);
                	wholeTestCases.get(i).setCondRedundancy(bd.doubleValue());
                }
                else
                	wholeTestCases.get(i).setCondRedundancy(SuiteR);
                wholeTestCases.get(i).setCondCoveredItem(ts);
            }
    	}
    }
    /*
    ...
    public void testSessionContainerRemoved(final TSContainerInfo tscInfo) {
        ManualRedundancyView.runInUI(new Runnable() {
            public void run() {
                int index;
//                if(!ManualRedundancyView.this.combo.isDisposed()) {
//                    index = ManualRedundancyView.this
//                                            .getVisTSCInfos().indexOf(tscInfo);
//                    ManualRedundancyView.this.getVisTSCInfos().remove(tscInfo);
//                    ManualRedundancyView.this.removeExpandedElements(tscInfo);
//                    if(index != -1) {
//                        ManualRedundancyView.this.combo.remove(index);
//                        ManualRedundancyView.this.combo.pack(true);
//                    } else {
//                        ManualRedundancyView.this.showErrorInconsistency();
//                    }
//                    if(ManualRedundancyView.this.getVisTSCInfos().isEmpty()) {
//                        ManualRedundancyView.this.noTSCinCombo();
//                    }
//                }
            }
        }, false);
    ...
    public void testSessionContainerActivated(
        ...
        ManualRedundancyView.runInUI(new Runnable() {
            public void run() {
                if(!ManualRedundancyView.this.viewer.getControl()
                                .isDisposed()) {
                    if(tscInfo != null) {
                        int index = ManualRedundancyView.this.getVisTSCInfos()
...
//                                ManualRedundancyView.this.combo.pack(true);
                    ...
                    } else if(!ManualRedundancyView.this
        ...
        ManualRedundancyView.runInUI(new Runnable() {
            public void run() {
                final String[] updateProps = {
                        RedundancyViewerFactory.UPDATE_PROPERTY_NAME,
                        RedundancyViewerFactory.UPDATE_PROPERTY_COMMENT
                        /*
                ...
                if(!ManualRedundancyView.this.viewer.getControl().isDisposed()) {
                    if(tscInfo.getTestSessionContainer()
                            == ManualRedundancyView.this.getVisTSC()) {
                        if(changeType == ChangeType.CHANGE) {
                            ManualRedundancyView.this.viewer.update(testCase,
                                            updateProps);
                        } else if(changeType == ChangeType.ADD) {
                            ManualRedundancyView.this.viewer.refresh(
                                    testCase.getTestSession(), false);
                            ManualRedundancyView.this.viewer.reveal(testCase);
                        } else if(changeType == ChangeType.REMOVE) {
                            if(ManualRedundancyView.this.getVisTestCases()
                                    .remove(testCase)) {
                                ManualRedundancyView.this.viewer.setCheckedElements(
                                        ManualRedundancyView.this.getVisTestCases()
                                                .toArray());
                            }
                            ManualRedundancyView.this.viewer.refresh(
                                    testCase.getTestSession(), false);
            ...
            public void run() {
                ...
                if(!ManualRedundancyView.this.viewer.getControl().isDisposed()
                        && tscInfo.getTestSessionContainer()
                                == ManualRedundancyView.this.getVisTSC()) {
                    if(changeType == ChangeType.CHANGE) {
                        ManualRedundancyView.this.viewer.update(testSession,
                                updateProps);
                    } else if(changeType == ChangeType.ADD) {
                        ...
                        for(TestCase testCase : testSession.getTestCases()) {
                            curTestCaseRemoved = ManualRedundancyView.this
            ...
            public void run() {
                if(!ManualRedundancyView.this.viewer.getControl().isDisposed()
                        && tscInfo.getTestSessionContainer()
                                == ManualRedundancyView.this.getVisTSC()) {
                    if(!ManualRedundancyView.this.getVisTestCases()
                            .equals(tscInfo.getRedundantTestCases())) {
                        ManualRedundancyView.this.getVisTestCases().clear();
                        ManualRedundancyView.this.getVisTestCases()
                                .addAll(tscInfo.getRedundantTestCases());
                        ManualRedundancyView.this.viewer
                                .setCheckedElements(
                                        tscInfo.getRedundantTestCases().toArray());
                    }
                }
            }
        }, false);
        ...
        runInUI(new Runnable() {
            public void run() {
                TSContainerInfo visTSCInfo
                        = ManualRedundancyView.this.getVisTSCInfo();
                if(visTSCInfo != null
    ...
    private final class ViewerCheckboxListener implements ICheckStateListener {
        ...
        public void checkStateChanged(CheckStateChangedEvent event) {
            ...
            .getTSContainerManager().getActiveTSContainer();
            CodeCoverPlugin.getDefault().getTSContainerManager()
                   .setRedundantTestCases(activeTSContainer.getActiveTestCases(),ManualRedundancyView.this.getVisTestCases());
        }
    }
    ...
    private final class ComboSelectionListener extends SelectionAdapter {
        ...
        public void widgetSelected(SelectionEvent event) {
            final TSContainerInfo selectedTSCInfo
                    = ManualRedundancyView.this.selectedTSContainer();
            if(selectedTSCInfo != null) {
                ...
                try {
                    ...
                    if(ManualRedundancyView.this.getVisTSCInfo() != null) {
                        int index = ManualRedundancyView.this
...
//                        ManualRedundancyView.this.combo.select(
//                                ManualRedundancyView.this.combo
//                                        .getItemCount()-1);
                    }
                    if(e.getCause() instanceof FileLoadException) {
                        MessageDialog.openError(
                                ManualRedundancyView.this.getSite().getShell(),
...
//        ManualRedundancyView.this.combo.add(
//                COMBO_ENTRY_NO_TEST_SESSION_CONTAINER_ACTIVE);
//        ManualRedundancyView.this.combo.select(
//                ManualRedundancyView.this.combo.getItemCount()-1);
//        this.combo.pack(true);
    }
    ...
    private void showErrorInconsistency() {
        ...
        ManualRedundancyView.this.logger.fatal(
    ...
    private class TSCDeleteAction extends Action implements IMenuCreator {
        ...
        public void run() {
            ...
            boolean deleteTSC = MessageDialog.openQuestion(
                    ManualRedundancyView.this.viewer.getControl().getShell(),
    ...
    private class MultipleTSCsDeleteAction extends Action {
        ...
        @Override
        public void run() {
            (new DeleteTSCsSelectDialog(ManualRedundancyView.this.getSite()
                    .getShell())).open();
        }
    }
        ...
        Action action = new Action() {
            ...
            public void run() {
                ...
                mergeWizard = new MergeWizard(
                        ManualRedundancyView.this.getVisTSC(),
                        (IStructuredSelection)ManualRedundancyView.this.viewer
                                .getSelection());
    ...
    private Action makeSelectAllAction() {
        Action action = new Action() {
            ...
            public void run() {
                ...
                allTSandTC.addAll(ManualRedundancyView.this.getVisTSC()
                        .getTestSessions());
                // add all test cases
                for(TestSession testSession : ManualRedundancyView.this
    ...
    private Action makeActivateAllAction() {
        Action action = new Action() {
            ...
            public void run() {
                ...
                .getTSContainerManager().getActiveTSContainer();
                CodeCoverPlugin.getDefault().getTSContainerManager()
                        .setRedundantTestCases(activeTSContainer.getActiveTestCases(),
                                ManualRedundancyView.this.getVisTestCases());
            }
        };
