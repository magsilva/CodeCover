...
public class CoverageAnnotationModel implements IAnnotationModel {
    ...
    protected void clear(AnnotationModelEvent event) {
        for (Iterator<EclPositionedAnnotation> i = annotations.iterator(); i.hasNext();) {
            EclPositionedAnnotation ca = i.next();
    ...
    public void connect(@SuppressWarnings("hiding")IDocument document) {
        ...
        for (EclPositionedAnnotation ca : annotations) {
            try {
    ...
    public Iterator<EclPositionedAnnotation> getAnnotationIterator() {
