...
/**
 * Represents a known test session container to save memory, only the location
 * of its file, its ID, and its date are saved.
 * <p>
 * In the documentation of this class the term <em>represented test session
 * container</em> is used for a <code>TestSessionContainer</code> which is
 * represented by an object of this class.
 */
public class TSContainerInfo implements TSContainerHandle {

    /**
     * File of the <code>TestSessionContainer</code> represented by this object.
     * Its path is used as the unique identifier for this object and the
     * corresponding <code>TestSessionContainer</code>.
     */
    private IFile file;
    ...
    /**
     * The name of this represented test session container, which is unique
     * during a session of Eclipse.
     */
    ...
    /**
     * Constructs a <code>TSContainerInfo</code>-representation of a test
     * session container with the given ID, date and name and which is contained
     * in the given file.
     * 
        ...
        synchronized(this.lockFile) {
            if(!file.getFullPath().equals(this.file.getFullPath())) {
                throw new IllegalArgumentException(
                        "file must have the same path");               //$NON-NLS-1$
            }
            this.file = file;
        }
    }
    ...
    /**
     * Stores the given (active) test cases represented as a list of
     ...
     *                  active test cases to remove/deactivate
     */
     ...
     * 
     * @param sync  <code>true</code> if the represented test session
     *              container is synchronized with its <code>IFile</code>,
    ...
    /**
     * Adds the given listener to receive events from the associated
     ...
     * associated <code>TSContainerInfo</code>.
     * 
     * @param listener  the listener
     */
     ...
     * doesn't have to be unique. Use the path of the file ({@link #getPath()}),
     * if you need a unique identifier for the represented test session
    ...
    /**
     * Returns the active test cases represented by an immutable copy of the
     * list of <code>TestCaseInfo</code>s.
     * 
     * @return  the active test cases represented by a list of
     *          <code>TestCaseInfo</code>s.
     */
    ...
    /**
     * Returns whether this <code>TSContainerInfo</code> equals the given
     * object.
     * <p>
     * This <code>TSContainerInfo</code> is equal to the given object if the
     * given object is a <code>TSContainerHandle</code> and both have the
     * same path ({@link #getPath()}). See {@link IPath#equals(Object)} for
     * equality of paths.
     * 
    ...
    /**
     * Compares this object with the given represented test session container
     * by the name of the project they belong to, by their dates and by their
     * names.
     */
    public int compareTo(TSContainerHandle tscHandle) {
        int cProjectName, cDate;
        if((cProjectName = this.getProject().getName().compareTo(
                tscHandle.getProject().getName())) != 0) {
            return cProjectName;
