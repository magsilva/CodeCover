...
/**
 * Represents an active <code>TestSessionContainer</code> and contains a
 * reference to the represented test session container and a list of the active
 * test cases of the represented test session container. An object of this
 * class isn't updated after being created, e.g. the set of active test
 * cases contained in an object of this class isn't adapted to later changes
 * in the selection of active test cases.
 * <p>
 * This class doesn't extend <code>TSContainerInfo</code> because it should be
 * used as a throw-away-object, it is only intended to be used as long as the
 * represented test session container is active and not as a replacement
 * for a TSContainerInfo. The reason for this is that objects of this class
 * contain a reference to a test session container and as long as this reference
 * exists the test session container resides in main memory. This problem is
 * also the reason why <code>TSContainerHandle</code> isn't public.
 * <p>
 * In the documentation of this class the term <em>represented test session
 * container</em> is used for the active <code>TestSessionContainer</code>
 * which is represented by an object of this class.
 */
public class ActiveTSContainerInfo implements TSContainerHandle {
    ...
    /**
     * Returns the restored active test cases of the represented test session
     ...
     *         container.
     */
    public static Set<TestCase> restoreActiveTestCases(TSContainerInfo tscInfo,
     ...
     * 
     * @return  the <code>TSContainerInfo</code>-representation of the
     *          represented test session container
     */
    ...
    /**
     * Returns an immutable copy of the set of active <code>TestCase</code>s
     * of the represented test session container.
     * 
     * @return  an immutable copy of the set of active
    ...
    /**
     * Returns the ID of the represented test session container, do
     * <em>not</em> use this ID as a unique identifier for this
     * representation of a test session container or for the represented
     * test session container itself, because the same test session
     * container can appear twice in the workspace (e.g., if the user
     * imported it two times).
     * <p>
     * A call to this method is equal to calling
     * <code>ActiveTSContainerInfo.getTSContainerInfo().getId()</code>. 
     * 
    ...
    /**
     * Returns the path of the file of the represented test session
     * container relative to the containing workspace. This path is used as
     * the unique identifier of this representation of a test session
     * container and the represented test session container itself.
     * <p>
     * A call to this method is equal to calling
     * <code>ActiveTSContainerInfo.getTSContainerInfo().getPath()</code>.
     * 
     * @return  the path of the file of the represented test session
     *          container.
     */
    ...
    /**
     * Returns the name of the represented test session container. This name
     * doesn't have to be unique (use the path of the file
     * ({@link #getPath()}), if you need a unique identifier for the
     * represented test session container or this representation of a test
     * session container).
     * <p>
     * A call to this method is equal to calling
    ...
    /**
     * Returns the project the represented test session container belongs
     * to.
     * <p>
     * A call to this method is equal to calling
     * <code>ActiveTSContainerInfo.getTSContainerInfo().getProject()</code>.
     * 
     * @return  the project the represented test session container belongs
     *          to.
     */
    ...
    /**
     * Returns whether the represented test session container is in sync
     * with its file in the workspace.
     * <p>
     * A call to this method is equal to calling
     * <code>ActiveTSContainerInfo.getTSContainerInfo().isSynchronized()</code>.
     * 
     * @return  <code>true</code> if the represented test session container
     *          is synchronized with its <code>IFile</code>,
    ...
    /**
     * Returns whether this object equals the given object. A call to this
    ...
    /**
     * Compares to represented test session containers by the name of the
     * project they belong to and by their dates.
     * <p>
     * A call to this method is equal to calling
     * <code>ActiveTSContainerInfo.getTSContainerInfo().compareTo(
    ...
    public String toString() {
        return String.format(
                "%s (%2$td.%2$tm.%2$tY %2$tH:%2$tM:%2$tS)",        //$NON-NLS-1$
                        getId(),
                        getDate());
    }
