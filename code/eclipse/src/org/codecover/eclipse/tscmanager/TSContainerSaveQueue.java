...
/**
 * The queue which contains the save operations which are queued because they
 * can't be performed instantly when they are a reaction on a project-close
 * event. The reason for this is that the workspace is locked during this event.
 * The save operation is represented as a pair of the
 * <code>TestSessionContainer</code> to save and its
...
public class TSContainerSaveQueue {
    ...
    private static final String MONITOR_SAVING_QUEUED_TEST_SESSION_CONTAINER = Messages
            .getString("TSContainerSaveQueue.MONITOR_SAVING_QUEUED_TEST_SESSION_CONTAINER"); //$NON-NLS-1$
    ...
    TSContainerSaveQueue(TSContainerStorage tscStorage) {
        if(tscStorage == null) {
            throw new NullPointerException(
                    "tscStorage mustn't be null");              //$NON-NLS-1$
        }
        
        this.tscStorage = tscStorage;
        this.queue = Collections.synchronizedMap(
                new HashMap<TSContainerInfo, TestSessionContainer>());
    ...
    void queueSave(TSContainerInfo tscInfo, TestSessionContainer tsc) {
        ...
        /*
         * if the test session container is already in the queue its entry
    ...
    /**
     * Locking of <code>TSContainerManager</code> must be done by the calling
     * context.
     */
    boolean saveQueued(IProgressMonitor monitor) throws CancelException,
            TSCQueuedSaveException {
        ...
        /*
         * copy keyset to avoid problems due to modifications of queue while
         * iterating over its keyset. if we wouldn't copy it, we would have to
         * put the whole for-loop inside the sync-block, which would block the
        ...
        monitor.beginTask(MONITOR_SAVING_QUEUED_TEST_SESSION_CONTAINER,
                queuedTSCInfos.size() * 1 * monitorScale);
        if(monitor.isCanceled()) {
            throw new CancelException(cancelDescr + "None.");                              //$NON-NLS-1$
        }
        for(TSContainerInfo tscInfo : queuedTSCInfos) {
            try {
                this.saveQueued(tscInfo, new SubProgressMonitor(monitor,
                        1 * monitorScale));
            }
            /*
             * catch TSCFileCreateException, FileSaveException and
             * OutOfMemoryError, log them and collect them in a list which will
             * be thrown as a TSCQueuedSaveException
             */
            catch(Throwable t) {
                exceptions.add(t);
     ...
     * context.
     */
    boolean saveQueued(IProject project, IProgressMonitor monitor)
            throws TSCQueuedSaveException {
        Set<TSContainerInfo> queuedTSCInfos;
        List<Throwable> exceptions = new ArrayList<Throwable>();
        boolean performedSave = false;
        final int monitorScale = 1000;
        ...
        for(TSContainerInfo tscInfo : queuedTSCInfos) {
            if(tscInfo.getFile().getProject().equals(project)) {
                try {
                    /*
                     * perform the pending save of the test session container,
                     * if there is one
                     */
                    this.saveQueued(tscInfo, new SubProgressMonitor(monitor,
                            1 * monitorScale));
                ...
                /*
                 * catch TSCFileCreateException, FileSaveException and
                 * OutOfMemoryError, log them and collect them in a list which
                 * will be thrown as a TSCQueuedSaveException
                 */
                catch(Throwable t) {
                    exceptions.add(t);
                    logger.error("Error while performing queued" + //$NON-NLS-1$
    ...
    /**
     * Performs the pending save (if there is any) of a test session container.
     * There won't be any retries if the saving fails (the save is removed from
     * the queue in any case, success or failure). Locking of
     ...
     *                  test session container to save
     * 
     * @throws TSCFileCreateException
     * (see {@link TSContainerStorage#saveTSContainer(TestSessionContainer, 
    ...
    void saveQueued(TSContainerInfo tscInfo, IProgressMonitor monitor)
        ...
        try { // this try only ensures that the monitor is left done()
            ...
            logger.debug("Performing queued save: "                //$NON-NLS-1$
                    + tscInfo.getFile().getFullPath().toString());
            /* may throw TSCFileCreateException, FileSaveException,
             * OutOfMemoryError
             */ 
            this.tscStorage.saveTSContainer(tsc, tscInfo, false,
