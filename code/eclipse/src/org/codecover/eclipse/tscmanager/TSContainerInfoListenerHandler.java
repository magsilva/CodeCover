...
/**
 * Handles the <code>TSContainerInfoListener</code>s of a
 * <code>TSContainerInfo</code> and contains methods to fire events which
 * signalize change of the associated <code>TSContainerInfo</code>.
 * 
...
class TSContainerInfoListenerHandler {
    ...
    /**
     * The <code>TSContainerInfo</code> this handler belongs to. Currently this
     * field is not needed, but may come in handy when new events to propagate
     * are added.
     */
        ...
        for(TSContainerInfoListener listener : listenersCopy) {
            try {
                ...
                CodeCoverPlugin.getDefault().getLogger().error(
                        "Error while notifying" +                  //$NON-NLS-1$
